    ...
    <div className="content" style={{ display:"grid", gridTemplateColumns:"var(--master-w) 1fr", padding:0, overflow:"hidden" }}>
      ...
      <aside style={{ borderRight:"1px solid var(--line-soft)", overflow:"auto", background:"var(--bg-1)" }}>
        <div className="row between center" style={{ padding:"15px 14px 10px" }}>
          ...
          <button className="icon-btn sm" title={tr('New object type')}><Icon name="plus" size={15}/></button>
        </div>
        <div style={{ padding:"0 8px 16px" }}>
          ...
          <button className="auth-new"><Icon name="plus" size={14}/>{tr('New object type')}</button>
        </div>
        <style>{`
      ...
      <div style={{ display:"flex", flexDirection:"column", minHeight:0 }}>
        ...
        <div className="row between center" style={{ padding:"12px 24px", borderBottom:"1px solid var(--line-soft)", flex:"none", background: dirty?"color-mix(in oklab, var(--warn) 8%, var(--bg-1))":"var(--bg-1)" }}>
          <div className="row gap-12 center">
            ...
            <div>
              <div className="row gap-8 center"><span className="serif" style={{ fontSize:18, fontWeight:600 }}>{tr(t.name)}</span><Badge kind="">{tr('type')}</Badge></div>
              <div className="t-faint mono" style={{ fontSize:11 }}>{tr('{count} objects · {props} properties · {links} links', { count: t.count.toLocaleString(), props: props.length, links: links.length })}</div>
            </div>
          </div>
          <div className="row gap-8 center">
            {dirty && <span className="mono" style={{ fontSize:11, color:"var(--warn)" }}>● {tr('unsaved schema changes')}</span>}
            <button className="btn sm" onClick={()=>{ setSchema(SEED_SCHEMA); setDirty(false); }} disabled={!dirty} style={{ opacity:dirty?1:.5 }}>{tr('Revert')}</button>
            <button className="btn primary sm" onClick={()=>setDirty(false)}><Icon name="check" size={14}/>{tr('Publish')}</button>
          </div>
        </div>
        ...
        <div className="content" style={{ padding:"var(--page-py) var(--page-px) 60px" }}>
          <div style={{ maxWidth:920, margin:"0 auto" }} className="fade-in" key={sel}>
            ...
            <div className="row between center" style={{ marginBottom:12 }}>
              <div className="eyebrow">{tr('Schema · properties')}</div>
              <button className="btn ghost sm" onClick={addProp}><Icon name="plus" size={13}/>{tr('Add property')}</button>
            </div>
            <div className="card" style={{ overflow:"hidden", marginBottom:28 }}>
              <div className="auth-head row" style={{ padding:"9px 14px", borderBottom:"1px solid var(--line)", gap:12 }}>
                <span className="eyebrow" style={{ flex:"1 1 40%" }}>{tr('Name')}</span>
                <span className="eyebrow" style={{ flex:"1 1 28%" }}>{tr('Type')}</span>
                <span className="eyebrow" style={{ flex:"1 1 24%" }}>{tr('Constraint')}</span>
                <span style={{ width:30 }} />
              </div>
              {props.map((p,i)=>(
                <div key={i} className="row center" style={{ padding:"9px 14px", borderBottom: i<props.length-1?"1px solid var(--line-soft)":"none", gap:12 }}>
                  ...
                  <button className="icon-btn sm" title={tr('Delete property')} onClick={()=>delProp(i)} style={{ width:30,height:30 }}><Icon name="x" size={14}/></button>
                </div>
              ))}
              {props.length===0 && <div className="t-faint" style={{ padding:"18px", textAlign:"center", fontSize:13 }}>{tr('No properties yet — add one.')}</div>}
            </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:24 }}>
              ...
              <div>
                <div className="row between center" style={{ marginBottom:12 }}>
                  <div className="eyebrow">{tr('Link types')}</div>
                  <button className="btn ghost sm"><Icon name="plus" size={13}/>{tr('Link')}</button>
                </div>
                    ...
                    return (
                      <div key={l.id} className="card" style={{ padding:"11px 13px" }}>
                        <div className="row center gap-8">
                          ...
                          <span className={"tc "+TYPE_BY_ID[other].cls+" row gap-6 center"}><span className="type-dot"/><span style={{ fontSize:12.5, color:"var(--text)", fontWeight:500 }}>{tr(TYPE_BY_ID[other].name)}</span></span>
                        </div>
                      </div>
                    );
                  })}
                  {links.length===0 && <div className="t-faint" style={{ fontSize:12.5, padding:"4px 2px" }}>{tr('No links on this type.')}</div>}
                </div>
              </div>
              ...
              <div>
                <div className="row between center" style={{ marginBottom:12 }}>
                  <div className="eyebrow">{tr('Action schemas')}</div>
                  <button className="btn ghost sm" onClick={()=>go && go("actions")}>{tr('Open Actions')} <Icon name="arrowRight" size={13}/></button>
                </div>
                <div className="col gap-8">
                    ...
                    <div key={k} className="card" style={{ padding:"11px 13px" }}>
                      <div className="row between center">
                        <div className="row gap-9 center">
                          ...
                          <div><div style={{ fontSize:13, fontWeight:600 }}>{tr(a.name)}</div><div className="t-faint mono" style={{ fontSize:10.5 }}>{tr('{n} params', { n: a.params.length })} · {a.approval.length?a.approval.join("+"):tr('no approval')}</div></div>
                        </div>
                      </div>
                    </div>
                  ))}
                  {boundActions.length===0 && <div className="t-faint" style={{ fontSize:12.5, padding:"4px 2px" }}>{tr('No action types target this object.')}</div>}
                </div>
              </div>
            </div>
          </div>
