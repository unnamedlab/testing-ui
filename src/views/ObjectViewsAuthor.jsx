...
export function ObjectViewsAuthor() {
  ...
  return (
    <div className="content" style={{ display: "grid", gridTemplateColumns: "228px 1fr 320px", padding: 0, overflow: "hidden" }}>
      {/* type list */}
      <aside style={{ borderRight: "1px solid var(--line-soft)", overflow: "auto", background: "var(--bg-1)" }}>
        <div style={{ padding: "15px 14px 10px" }}><div className="eyebrow">Object types</div></div>
      ...
      <div style={{ display: "flex", flexDirection: "column", minHeight: 0 }}>
        <div className="row between center" style={{ padding: "12px 22px", borderBottom: "1px solid var(--line-soft)", flex: "none", background: dirty ? "color-mix(in oklab, var(--warn) 8%, var(--bg-1))" : "var(--bg-1)" }}>
          <div className="row gap-12 center">
            ...
            <div><div className="row gap-8 center"><span className="serif" style={{ fontSize: 17, fontWeight: 600 }}>{t.name} view</span><Badge kind="">{layout.length} cards</Badge></div>
              <div className="t-faint mono" style={{ fontSize: 11 }}>Entity 360 layout</div></div>
          </div>
          <div className="row gap-8 center">
            {dirty && <span className="mono" style={{ fontSize: 11, color: "var(--warn)" }}>● unsaved</span>}
            <button className="btn sm" onClick={() => { setLayouts(DEFAULT_LAYOUT); setHighlight({}); setDirty(false); }} disabled={!dirty} style={{ opacity: dirty ? 1 : .5 }}>Reset</button>
            <button className="btn primary sm" onClick={() => setDirty(false)}><Icon name="check" size={14} />Publish view</button>
          </div>
        </div>

        <div className="content" style={{ padding: "20px 24px 50px" }}>
          <div style={{ maxWidth: 620, margin: "0 auto" }} className="fade-in" key={sel}>
            <div className="eyebrow" style={{ marginBottom: 10 }}>Cards in this view · drag order with ▲▼</div>
                ...
                return (
                  <div key={id} className="card" style={{ padding: "12px 14px", display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div className="t-faint" style={{ fontSize: 11.5 }}>{c.desc}</div>
                    </div>
                    {id === "props" && <span className="badge accent" style={{ fontSize: 9.5 }}>{hi.length} fields</span>}
            ...
            <div className="eyebrow" style={{ marginBottom: 10 }}>Add a card</div>
            <div className="row gap-8 wrap" style={{ marginBottom: 26 }}>
              {CARD_LIBRARY.filter(c => !layout.includes(c.id)).map(c => (
                <button key={c.id} className="chip" onClick={() => toggleCard(c.id)}><Icon name={c.icon} size={13} />{c.name}</button>
              ))}
              {CARD_LIBRARY.every(c => layout.includes(c.id)) && <span className="t-faint" style={{ fontSize: 12.5 }}>All cards in use.</span>}
            </div>

            {/* highlighted properties picker */}
            <div className="eyebrow" style={{ marginBottom: 10 }}>Highlighted properties</div>
            <div className="row gap-8 wrap">
              ...
              {props.length === 0 && <span className="t-faint" style={{ fontSize: 12.5 }}>No schema properties — define them in Author.</span>}
            </div>
          </div>
        </div>
      </div>
      ...
      <aside style={{ borderLeft: "1px solid var(--line-soft)", background: "var(--bg-inset)", overflow: "auto", padding: "16px 16px 40px" }}>
        <div className="eyebrow" style={{ marginBottom: 12 }}>Preview · Entity 360</div>
          ...
          {layout.map((id, i) => {
            ...
            if (id === "summary") return (
              <div key={id} style={{ padding: 14, borderBottom: "1px solid var(--line-soft)" }}>
                <div className="row gap-10 center"><TypeGlyph type={sel} size={36} />
                  <div><div style={{ fontSize: 14, fontWeight: 600 }}>{t.name} · sample</div><div className="t-faint" style={{ fontSize: 11 }}>{t.name}</div></div></div>
              </div>
            );
            if (id === "props") return (
              <div key={id} style={{ padding: 14, borderBottom: "1px solid var(--line-soft)" }}>
                <div className="eyebrow" style={{ marginBottom: 8, fontSize: 9.5 }}>Highlighted</div>
            ...
            return (
              <div key={id} style={{ padding: "11px 14px", borderBottom: i < layout.length - 1 ? "1px solid var(--line-soft)" : "none" }}>
                <div className="row gap-8 center"><span style={{ color: "var(--accent)" }}><Icon name={c.icon} size={14} /></span><span style={{ fontSize: 12.5, fontWeight: 600 }}>{c.name}</span></div>
                <div style={{ height: 26, marginTop: 8, borderRadius: 6, background: "var(--bg-2)", border: "1px dashed var(--line)" }} />
              </div>
            );
          })}
        </div>
        <div className="t-faint mono" style={{ fontSize: 10, marginTop: 12, lineHeight: 1.6 }}>This is what an analyst sees when they open a {t.name} in Entity 360.</div>
      </aside>
    </div>
  );
}
