...
/* ============================================================
   AXIOM — Sources (Integrate · data ingestion) — UX-01 i18n
   ============================================================ */
  ...
  return (
    <div className="content" style={{ padding: "var(--page-py) var(--page-px) 60px" }}>
      <div style={{ maxWidth: 1080, margin: "0 auto" }} className="fade-in">
        {/* header */}
        <PageHeader eyebrow={t('Integrate')} title={t('Sources')}>
          <button className="btn primary" onClick={() => setWizard(true)}><Icon name="plus" />{t('Add source')}</button>
        </PageHeader>
        <p className="t-dim" style={{ fontSize: 14, margin: "0 0 22px", maxWidth: "74ch" }}>
          {t('The ingestion layer — connect upstream systems and keep them in sync.')}
          <span className="t-faint"> {t('Ingest health answers “is the connector live and the source fresh?”. Transform health lives in')} </span>{t('Pipelines › Data Health')}.
        </p>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12, marginBottom: 24 }}>
          {[["Connectors", stats.total, "database"], ["Healthy", stats.healthy + "/" + stats.total, "check"],
            ...
            <div key={k} className="card" style={{ padding: 16 }}>
              <div className="row between center"><div className="eyebrow">{t(k)}</div><span className="t-faint"><Icon name={ic} size={15} /></span></div>
        ...
        <div className="row gap-8 center" style={{ marginBottom: 16, flexWrap: "wrap" }}>
          ...
          <span className="badge accent"><span className="dt" />{t('Sources')}</span>
          <span className="t-faint"><Icon name="arrowRight" size={13} /></span>
          <span className="badge">{t('Pipelines')}</span>
          <span className="t-faint"><Icon name="arrowRight" size={13} /></span>
          <span className="badge">{t('Code')}</span>
          <span className="t-faint"><Icon name="arrowRight" size={13} /></span>
          <span className="badge">{t('Models')}</span>
        </div>
          ...
          {shown.map(s => (
            <div key={s.id} className="card" style={{ padding: 16 }}>
              <div className="row between center" style={{ marginBottom: 12 }}>
                ...
                <Badge kind={s.status === "healthy" ? "ok" : "warn"} dot>{s.status === "healthy" ? t('connected') : t(s.status)}</Badge>
              </div>
              ...
              <div className="row gap-8 center" style={{ marginBottom: 12, fontFamily: "var(--font-mono)", fontSize: 10.5, color: "var(--text-faint)" }}>
                <span style={{ color: s.status === "healthy" ? "var(--ok)" : "var(--warn)" }}>● {t('connector')}</span>
                <Icon name="arrowRight" size={12} />
                <span>{t('upstream · fresh {fresh}', { fresh: s.fresh })}</span>
              </div>

              <div className="row between" style={{ marginBottom: 12 }}>
                {[["Records", s.records], ["Cadence", s.cadence], ["Freshness", s.fresh]].map(([k, v]) => (
                  <div key={k}><div className="t-faint" style={{ fontSize: 10.5 }}>{t(k)}</div><div className="mono" style={{ fontSize: 12.5, color: "var(--text)" }}>{v}</div></div>
                ))}
              </div>
              <div className="row gap-8 center">
                <span className="t-faint" style={{ fontSize: 11, width: 72 }}>{t('ingest health')}</span>
