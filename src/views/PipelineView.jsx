...
/* ============================================================
   AXIOM — PipelineView
   Pipeline builder: stage graph raw → clean → ontology.
   "Pipelines" (build) tab of PipelinesWorkbench. (UX-01.)
   ============================================================ */
const STAGES = [
  { id: "ingest", name: "swift_ingest", layer: "Connector", out: "txn_raw", status: "healthy", icon: "download" },
  ...
  return (
    <div className="content" style={{ padding: "var(--page-py) var(--page-px) 60px" }}>
      <div style={{ maxWidth: 1080, margin: "0 auto" }} className="fade-in">
        <PageHeader eyebrow={t('Integrate')} title={<>{t('Pipeline')} · blackfrost</>}>
          <button className="btn"><Icon name="history" size={15} />{t('History')}</button>
          <button className="btn primary"><Icon name="play" size={15} />{t('Run')}</button>
        </PageHeader>
              ...
              <div key={s.id} className="row" style={{ alignItems: "center" }}>
                <div className="panel" style={{ width: 190, padding: 14, background: "var(--bg-1)" }}>
                  <div className="row between center" style={{ marginBottom: 10 }}>
                    ...
                    <span className="badge" style={{ color: ST[s.status].c, borderColor: "transparent" }}><span className="dt" style={{ background: ST[s.status].c }} />{t(ST[s.status].l)}</span>
                  </div>
                  <div className="mono" style={{ fontSize: 12.5, fontWeight: 600, color: "var(--text)" }}>{s.name}</div>
                  <div className="t-faint" style={{ fontSize: 11, marginTop: 2 }}>{t(s.layer)}</div>
        ...
        <div className="t-faint mono" style={{ fontSize: 11, marginTop: 12 }}>{t('{n} stages · cadence */5 min · last run 5m ago', { n: STAGES.length })}</div>
      </div>
    </div>
  );
}
