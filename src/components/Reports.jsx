  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:140, background:"var(--scrim)", backdropFilter:"var(--scrim-blur)", display:"flex", flexDirection:"column" }}>
      ...
      <div className="no-print row between center" style={{ padding:"12px 20px", background:"var(--bg-1)", borderBottom:"1px solid var(--line)" }}>
        <div className="row gap-10 center"><Icon name="doc" size={18}/><span className="serif" style={{ fontSize:16 }}>Dossier · {headerName}</span><Badge kind="accent"><Icon name="sparkles" size={11}/>AI-assembled</Badge></div>
          ...
          <div style={{ padding:"40px 52px 52px" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", borderBottom:"2px solid #16181d", paddingBottom:18, marginBottom:26 }}>
              <div>
                ...
                <div className="serif" style={{ fontSize:30, fontWeight:600, letterSpacing:"-0.01em", lineHeight:1.1 }}>{headerName}</div>
            ...
            {blocks ? <DossierBlocks blocks={blocks} /> : (<>
...
// ---- Render the report editor's block model into the printable dossier, so an
// edited brief actually exports what the analyst sees (one content model). ----
const dlabel = { fontFamily:"var(--font-mono)", fontSize:10, letterSpacing:".08em", textTransform:"uppercase", color:"#6b7280", marginBottom:8 };
function DossierObjects({ ids, label }){
  const ents = (ids||[]).map(id=>ENTITY_BY_ID[id]).filter(Boolean);
  return (
    <div style={{ marginBottom:20 }}>
      {label && <div style={dlabel}>{label}</div>}
      <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12.5 }}>
        <thead><tr>{["Object","Type","Role","Risk"].map(h=><th key={h} style={dth}>{h}</th>)}</tr></thead>
        <tbody>{ents.map(e=>(
          <tr key={e.id}><td style={dtd}><b>{e.name}</b></td><td style={dtd}>{TYPE_BY_ID[e.type].name}</td><td style={dtd}>{e.sub}</td>
            <td style={{...dtd, textAlign:"right", fontWeight:600, color: e.risk>=80?"#b42318":"#b54708"}}>{e.risk}</td></tr>
        ))}</tbody>
      </table>
    </div>
  );
...
function DossierChart({ label }){
  const vals=[38,62,30,81,54,72,44,90,60]; const max=Math.max(...vals);
  return (
    <div style={{ marginBottom:20 }}>
      {label && <div style={dlabel}>{label}</div>}
      <div style={{ display:"flex", alignItems:"flex-end", gap:6, height:90, padding:"10px 12px", border:"1px solid #e5e7eb", borderRadius:6 }}>
        {vals.map((v,i)=><div key={i} style={{ flex:1, height:`${(v/max)*100}%`, background:"#0b3b4a", borderRadius:"3px 3px 0 0" }}/>)}
      </div>
    </div>
  );
}
export function DossierBlocks({ blocks }){
  return blocks.map((b,i)=>{
    if(b.type==="h") return <div key={i} className="serif" style={{ fontSize:17, fontWeight:600, margin:"22px 0 10px", color:"#16181d" }}>{b.text}</div>;
    if(b.type==="p") return <p key={i} style={{ ...dpara, marginBottom:14 }}>{b.text}</p>;
    if(b.type==="findings") return <ul key={i} style={{ ...dpara, paddingLeft:18, marginBottom:16 }}>{b.items.map((it,j)=><li key={j} style={{ marginBottom:7 }}>{it}</li>)}</ul>;
