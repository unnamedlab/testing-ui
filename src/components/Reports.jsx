import { ANALYSTS, CLASSIFICATION, CLASS_LEVELS } from '../data/data_ext.js';
...
/* ============================================================
   AXIOM — Dossier / report generator (printable) — UX-01 i18n
   ============================================================ */

export function DossierModal({ open, onClose, caseId }){
  const { t, lang } = useI18n();
  if(!open) return null;
  const c = PROJECT_BY_ID[caseId] || PROJECTS[0];
  ...
  const ink = CLASS_LEVELS[c.cls]?.ink || "#b54708";
  const keyEnts = ENTITIES.filter(e=>e.risk>=70).slice(0,6);
  const today = new Intl.DateTimeFormat(lang === 'es' ? 'es-ES' : 'en-US', { day: '2-digit', month: 'long', year: 'numeric', timeZone: 'UTC' }).format(new Date());
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:140, background:"var(--scrim)", backdropFilter:"var(--scrim-blur)", display:"flex", flexDirection:"column" }}>
      ...
      <div className="no-print row between center" style={{ padding:"12px 20px", background:"var(--bg-1)", borderBottom:"1px solid var(--line)" }}>
        <div className="row gap-10 center"><Icon name="doc" size={18}/><span className="serif" style={{ fontSize:16 }}>{t('Dossier')} · {t(c.name)}</span><Badge kind="accent"><Icon name="sparkles" size={11}/>{t('AI-assembled')}</Badge></div>
        <div className="row gap-8">
          <button className="btn" onClick={()=>window.print()}><Icon name="download"/>{t('Export PDF')}</button>
          <button className="btn ghost" onClick={onClose}><Icon name="x" size={16}/>{t('Close')}</button>
        </div>
      </div>
      ...
      <div style={{ flex:1, overflow:"auto", padding:"28px 20px", display:"flex", justifyContent:"center" }}>
        <div className="dossier" style={{ width:"100%", maxWidth:780, background:"#fff", color:"#16181d", borderRadius:6, boxShadow:"var(--shadow-3)", overflow:"hidden" }}>
          ...
          <div style={{ background:ink, color:"#fff", textAlign:"center", padding:"5px", fontFamily:"var(--font-mono)", fontSize:11, letterSpacing:".18em", fontWeight:600 }}>
            {c.cls}{' // '}{CLASSIFICATION.compartment}{' // '}{CLASSIFICATION.caveat}
          </div>
          ...
          <div style={{ padding:"40px 52px 52px" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", borderBottom:"2px solid #16181d", paddingBottom:18, marginBottom:26 }}>
              <div>
                <div style={{ fontFamily:"var(--font-mono)", fontSize:11, letterSpacing:".14em", color:"#6b7280", marginBottom:8 }}>{t('AXIOM · INTELLIGENCE BRIEF')}</div>
                <div className="serif" style={{ fontSize:30, fontWeight:600, letterSpacing:"-0.01em", lineHeight:1.1 }}>{t(c.name)}</div>
                <div style={{ fontSize:13, color:"#4b5563", marginTop:8 }}>{t('Prepared {today} · Lead: {lead} · Ref {ref}', { today, lead: ANALYSTS[c.lead].name, ref: `AXM-${c.id.toUpperCase()}-${caseRef(c.id)}` })}</div>
              </div>
            ...
            <Section h={t('1 · Executive summary')}>
              <p style={dpara}>{t(c.summary)} {t('Network risk is assessed at 84/100 (Critical). Confidence: High. {n} alerts remain open, of which 3 are critical and pending escalation to the financial-intelligence liaison.', { n: c.counts?.alerts ?? 0 })}</p>
            </Section>

            <Section h={t('2 · Key entities of interest')}>
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12.5 }}>
                <thead><tr>{["Object","Type","Role","Risk"].map(h=><th key={h} style={dth}>{t(h)}</th>)}</tr></thead>
                <tbody>
                  {keyEnts.map(e=>(
                    <tr key={e.id}>
                      <td style={dtd}><b>{e.name}</b></td>
                      <td style={dtd}>{t(TYPE_BY_ID[e.type].name)}</td>
            ...
            <Section h={t('3 · Financial findings')}>
              <p style={dpara}>{t('Three layering chains moved $20.4M between Aurora Trading FZE and Helios Maritime over 30 days, structured across 3–5 hops to obscure origin. The largest single transfer (TXN-88241, $4.82M) routed through correspondent accounts at two jurisdictions within 48 hours — a pattern consistent with deliberate trade-finance laundering.')}</p>
            </Section>

            <Section h={t('4 · Maritime findings')}>
              <p style={dpara}>{t('MV Blackfrost (IMO 9583217) recorded 14 AIS gaps in 90 days, the longest coinciding with a port call at Novorossiysk (RUNVS), a sanctioned terminal. Flag changed Panama → Cook Islands during the period; classification society suspended. Behaviour is indicative of cargo-origin concealment.')}</p>
            </Section>

            <Section h={t('5 · Assessment & recommendations')}>
              <ul style={{ ...dpara, paddingLeft:18, margin:0 }}>
                <li style={{ marginBottom:7 }}>{t('File a Suspicious Transaction Report for the Aurora→Helios chain.')}</li>
                <li style={{ marginBottom:7 }}>{t('Request beneficial-ownership records for Northwind Holdings (BVI).')}</li>
                <li style={{ marginBottom:7 }}>{t('Add MV Blackfrost and operator to the maritime watchlist; share with partner.')}</li>
              </ul>
            </Section>

            <div style={{ marginTop:30, paddingTop:16, borderTop:"1px solid #e5e7eb", fontSize:11, color:"#9ca3af", fontFamily:"var(--font-mono)" }}>
              {t('Generated by AXIOM · sources: AIS Vessel Feed, SWIFT MT103, Corporate Registry, OFAC/EU lists · lineage verified.')}
            </div>
          </div>
